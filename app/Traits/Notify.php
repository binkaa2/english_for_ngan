...
class Notify
{
   ...
   private static $api_access_key;
    ...
    {
        self::$app_id = env('NOTI_APP_ID','e5d82461-8987-4133-b080-7092d677f9fc');
        self::$rest_api_key = env('NOTI_REST_API_KEY','MzE1YzhmYzgtZGMwYS00ZGEyLWE3OTMtZWQzMDlkNmYxOTM2');
    ...
    public function sendNotifyToUser( $filter, $content,$url = null){
        $arr = [];
        $arr['filters'] = [];
        ...
        if($url){
            $button =  array(
                "id" => "like-button",
                "text" => "Check My List Order",
                "icon" => "http://www.clker.com/cliparts/z/g/U/q/e/l/check-mark-md.png",
                "url" => $url,
            );
            $response = $this->sendMessage($filter, $content, $button);
        }else{
            $response = $this->sendMessage($filter, $content);
        }
       return $response;
    }
    public function sendMessage( $filter, $content, $button = null) {
        $content_arr      = array(
            "en" => $content
        );
        $hashes_array = array();
        if ($button != null) array_push($hashes_array, $button);
        $fields = array(
